...
def read_data():
    ...
    word1 = parse_word(ser.read(2))
    word2 = parse_word(ser.read(2))
...
try:
    ...
    for i in range(data_points):
        word1, word2, word3, word4 = read_data()
        word1_data[i] = word1
        word2_data[i] = word2
        word3_data[i] = word3
        word4_data[i] = word4
    ...
    # Plot data
    ax1.plot(word1_data[10:])
    ax1.set_title('roll')
    ax1.set_xlabel('Sample')
    ax1.set_ylabel('Value')

    ax2.plot(word2_data[10:])
    ax2.set_title('output')
    ax2.set_xlabel('Sample')
    ax2.set_ylabel('Value')

    ax3.plot(word3_data[10:])
    ax3.set_title('setpoint')
    ax3.set_xlabel('Sample')
    ax3.set_ylabel('Value')

    ax4.plot(word4_data[1000:])
    ax4.set_title('setpoint')
